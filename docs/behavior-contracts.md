...
| R-007 | Tracing and debug recording are disabled by default, preserve execution and attempt correlation, cannot affect Runtime behavior, and accept no prompt, message, reasoning, Tool payload, credential, endpoint, or exception text | Core tracing and Runtime tracing/debug contracts |
