...
class GatewayProtocolContractTest {
    ...
    @Test
    fun requestWireHasExactV1AndNoCredentialEndpointOrHeaderSurface() {
        val encoded = codec.encodeCreateRequest(request())

        assertEquals(
            "{\"protocolVersion\":1,\"requestId\":\"session-1:0\",\"sessionId\":\"session-1\"," +
    ...
    fun decoderRejectsUnknownVersionUnknownEventAndUnknownField() {
        ...
        assertFailsWith<GatewayProtocolException> {
            codec.decodeEnvelope(encoded.replace("\"protocolVersion\":1", "\"protocolVersion\":2"))
        }
        assertFailsWith<GatewayProtocolException> {
    ...
    fun eventAndProblemOpaqueFieldsHaveStrictLimits() {
        ...
        assertFailsWith<GatewayProtocolException> {
            codec.decodeProblem(
                "{\"protocolVersion\":1,\"code\":\"invalid\",\"message\":\"safe\"," +
                    "\"retryAfterMillis\":-1}",
            )
        }
    }
